...
local function Save(player, vehicle)
    if Config.UseParkingZones then
        showmessage = false
    end
    ParkCar(player, vehicle)
    local vehicleProps = QBCore.Functions.GetVehicleProperties(vehicle)
...
RegisterNetEvent("qb-parking:client:park", function()
    isUsingParkCommand = true
end)
...
CreateThread(function()
	while true do
        if Config.UseParkingZones and Config.ShowZoneNames then
            Wait(0)
            local fd = true
            local pl = GetEntityCoords(PlayerPedId())
            if fd then
                fd = false
                for k, v in pairs(Config.ParkingZones) do
                    if #(pl - vector3(v.enter.x, v.enter.y, v.enter.z)) < 20 then
                        Draw3DText(v.enter.x, v.enter.y, v.enter.z, v.name, 0, 0.2, 0.2)
                        Draw3DText(v.enter.x, v.enter.y, v.enter.z - 0.5, Lang:t("info.park_cost", {cost = v.cost}), 0, 0.1, 0.1)
                        fd = true
                    end
                end
            else
...
CreateThread(function()
    if UseParkingSystem then
		while true do
            ...
            if IsControlJustReleased(0, Config.parkingButton) then
                if not InParking then
                    QBCore.Functions.Notify("You are not in a parking space.", "error", 2000)
                end
            end
			...
			if InParking and IsPedInAnyVehicle(player) then
				...
				if storedVehicle ~= false then
					DisplayHelpText(Lang:t("system.drive"))
                    if IsControlJustReleased(0, Config.parkingButton) then
                        isUsingParkCommand = true
                    end
                else
                    if parkData.notify == nil or parkData.notify then
                        DisplayHelpText(Lang:t("system.park"))
                        if IsControlJustReleased(0, Config.parkingButton) then
                            isUsingParkCommand = true
                        end
                    end
