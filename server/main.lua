...
QBCore.Functions.CreateCallback("qb-parking:server:savetrailer", function(source, cb, trailer)
	FindPlayerVehicles(GetCitizenid(Player), function(vehicles) -- free for all
		for k, v in pairs(vehicles) do
			if type(v.plate) and plate == v.plate then
				isFound = true
		...
		if isFound then
			MySQL.Async.fetchAll("SELECT * FROM player_parking_trailers WHERE citizenid = @citizenid AND plate = @plate", {
				['@citizenid'] = GetCitizenid(Player),
				['@plate']     = plate
			}, function(rs)
				if type(rs) == 'table' and #rs > 0 then
					...
					MySQL.Async.execute("INSERT INTO player_parking_trailers (citizenid, citizenname, plate, fuel, model, modelname, data, time) VALUES (@citizenid, @citizenname, @plate, @fuel, @model, @modelname, @data, @time)", {
						["@citizenid"]   = GetCitizenid(Player),
						["@citizenname"] = GetUsername(Player),
						["@plate"]       = plate,
						['@model']       = vehicleData.model,
					...
					TriggerClientEvent("qb-parking:client:addTrailer", -1, {
						plate       = plate, 
						citizenid   = GetCitizenid(Player), 
						citizenname = GetUsername(Player),
						model       = vehicleData.model,
...
-- Unpark the trailer
QBCore.Functions.CreateCallback("qb-parking:server:unparktrailer", function(source, cb, trailer)
	MySQL.Async.fetchAll("SELECT * FROM player_parking_trailers WHERE citizenid = @citizenid AND plate = @plate", {
		['@citizenid'] = GetCitizenid(Player),
		['@plate'] = plate
	}, function(rs)
		if type(rs) == 'table' and #rs > 0 and rs[1] then
			MySQL.Async.execute('DELETE FROM player_parking_trailers WHERE plate = @plate AND citizenid = @citizenid', {
				["@plate"]     = plate,
				["@citizenid"] = GetCitizenid(Player)
			})
